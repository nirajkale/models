...
def eval_all(
    ...
    save_final_config=False,
    **kwargs):
  """Run evaluation of all the checkpoint presennt in checkpoint dir & quit
  this is to avoid running eval script in parallel with training script
  ...
  if model_dir and save_final_config:
    tf.logging.info('Saving pipeline config file to directory {}'.format(model_dir))
    pipeline_config_final = create_pipeline_proto_from_configs(configs)
    config_util.save_pipeline_config(pipeline_config_final, model_dir)
  ...
  optimizer, _ = optimizer_builder.build(configs['train_config'].optimizer, global_step=global_step)

  # for latest_checkpoint in tf.train.checkpoints_iterator(checkpoint_dir, timeout=300, min_interval_secs=3600):
  #instead of getting the latest checkpoint, get all the checkpoints & iterate over them
  checkpoint_state = tf.train.get_checkpoint_state(checkpoint_dir)
  n_checkpoints = len(checkpoint_state.all_model_checkpoint_paths)
  tf.logging.info(f'Evluation will be done with batch_size: {eval_config.batch_size}')
  tf.logging.info(f'Total {n_checkpoints} checkpoints found for evaluation')
  for i, checkpoint_path in enumerate(checkpoint_state.all_model_checkpoint_paths):
    ckpt_name = os.path.basename(checkpoint_path)
    tf.logging.info(f'Loading checkpoint {ckpt_name} [{i+1}/{n_checkpoints}]')
    ckpt = tf.compat.v2.train.Checkpoint(step=global_step, model=detection_model, optimizer=optimizer)
    if eval_config.use_moving_averages:
      unpad_groundtruth_tensors = (eval_config.batch_size == 1 and not use_tpu)
      _ensure_model_is_built(detection_model, eval_input,unpad_groundtruth_tensors)
      optimizer.shadow_copy(detection_model)
    ckpt.restore(checkpoint_path).expect_partial()
    if eval_config.use_moving_averages:
      optimizer.swap_weights()
    tf.logging.info(f'Value of Global step being logged: {global_step}')
    ...
    tf.logging.info(f'Checkpoint eval completed')
